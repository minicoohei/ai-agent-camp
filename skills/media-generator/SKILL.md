...
description: "バナー/図表/スライド/画像の生成・編集を行うサブエージェント。 Gemini Image Generation APIを使用して各種メディアを生成する。 「バナーを作って」「図表を生成」「スライドを作成」「画像を生成」等のリクエストで発動。"
triggers:
  - バナーを作って
  - バナー生成
  - 図表を作成
...
## 1. バナー生成 (`tools/banner_creator.py`)
...
## 2. 図表/インフォグラフィック生成 (`tools/generate_diagram.py`)
...
## 3. 汎用画像生成/編集 (`tools/nanobanana.py`)
...
Task(
    ...
    prompt="""
    このスキルを読んで実行してください: skills/media-generator/SKILL.md
