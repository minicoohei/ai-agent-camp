...
description: |
  コンテンツのA/Bテスト・自己改善ループを支援するプランナースキル。
  市場トレンド調査 → 仮説生成 → 実験設計 → メトリクス収集 → 分析改善の全サイクルをカバー。
  以下のリクエストで使用:
  - 「A/Bテストを設計して」「投稿を改善したい」「コンテンツを最適化」
  - 「バズっている投稿を調べて仮説を立てて」「トレンド分析して投稿案を作って」
  - 「ツイートのパフォーマンスを分析」「エンゲージメント改善」
  - 「Typefullyで投稿のA/Bテスト」「メール件名のテスト設計」
dependencies:
  - requests>=2.28.0
