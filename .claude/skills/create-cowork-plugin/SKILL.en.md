...
description: "Skill for creating plugins from scratch during a Cowork session. Triggered by requests like 'create a plugin,' 'plugin development,' 'build a plugin,' etc. Generates .plugin files in Cowork mode."
...
```bash
zip -r /tmp/plugin-name.plugin /path/to/plugin-dir/. -x "*.DS_Store" && cp /tmp/plugin-name.plugin /path/to/outputs/plugin-name.plugin
```
