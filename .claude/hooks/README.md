...
PI パターン（英語・日本語）と危険操作（curl, eval, exec, rm -rf 等）の
**組み合わせ**でブロック。PI パターン単独では警告のみ（過検知防止）。
...
2. 正当な操作であれば `CLAUDE_GUARDRAILS_SKIP=1` で一時的にスキップ
3. 恒常的な誤検知の場合はパターンを修正
