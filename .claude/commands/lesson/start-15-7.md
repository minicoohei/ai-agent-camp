...
ミュージックビデオの品質をチェックしてください。

■ チェック対象: 生成されたMV動画
■ 重点項目:
- ビート同期: シーン切り替えが楽曲のビートに合っているか
- BGMフェードアウト: 末尾でブツ切りになっていないか
- A-roll/B-roll のバランス
```
...
```json
{
  ...
  "questions": [{
    ...
    "options": [
      {"id": "next_auto", "label": "次のセクション（/start-15-8 スライド解説動画）"},
