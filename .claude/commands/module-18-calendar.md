...
1. 直近の予定取得（例: `--days 7`）。
2. 必要なら空き時間検索（`free-slots` 相当）と、条件に合うイベント作成。
3. 自然言語指示（例: 来週の空きに 1on1）に沿ってスクリプトまたは `gog` で実行し、結果を要約する。

## 参照

- コース: `slideId=lesson-18-2`（例: `/ja/course/module-18?slideId=lesson-18-2`）
- スキル: `.claude/skills/google-workspace-ops/` の SKILL.md
